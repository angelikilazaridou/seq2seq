...
-- file location
cmd:option('-models_num', 'Number of models (if >1 then do ensemble)')
cmd:option('-model1', 'seq2seq_lstm_attn.t7.', [[Path to model .t7 file]])
cmd:option('-model2', 'seq2seq_lstm_attn.t7.', [[Path to model .t7 file]])
cmd:option('-model3', 'seq2seq_lstm_attn.t7.', [[Path to model .t7 file]])
cmd:option('-model4', 'seq2seq_lstm_attn.t7.', [[Path to model .t7 file]])
...
function generate_beam(models, models_num, initial, K, max_sent_l, source, gold)
  ...
  -- Backpointer table.
   ...
   local rnn_state_encs = {}
   local rnn_state_decs = {}
   local contexts = {}
   for m = 1, models_num do
      ...
      table.insert(contexts, context_protos[m][{{}, {1,source_l}}]:clone())
      rnn_state_encs[m] = {}
      for i = 1, #init_fwd_encs[m] do
        table.insert(rnn_state_encs[m], init_fwd_encs[m][i]:zero())
      end
      for t = 1, source_l do
        local encoder_input = {source_input[t], table.unpack(rnn_state_encs[m])}
        local out = models[m][1]:forward(encoder_input)
        rnn_state_encs[m] = out
        contexts[m][{{},t}]:copy(out[#out])
      end

      rnn_state_decs[m] = {}
      for i = 1, #init_fwd_decs[m] do
        table.insert(rnn_state_decs[m], init_fwd_decs[m][i]:zero())
      end

      if models_opt[m].init_dec == 1 then
        for L = 1, models_opt[m].num_layers do
          rnn_state_decs[m][L*2-1+models_opt[m].input_feed]:copy(
            rnn_state_encs[m][L*2-1]:expand(K, models_opt[m].rnn_size))
          rnn_state_decs[m][L*2+models_opt[m].input_feed]:copy(
            rnn_state_encs[m][L*2]:expand(K, models_opt[m].rnn_size))
        end
      end

      contexts[m] = contexts[m]:expand(K, source_l, models_opt[m].rnn_size)
   end
   ...
   while (not done) and (i < n) do
      ...
      for m = 1, models_num do
        ...
        local decoder_input 
        if models_opt[m].attn == 1 then
	         decoder_input =   {decoder_input_words, contexts[m], table.unpack(rnn_state_decs[m])}
        else
	         decoder_input =  {decoder_input_words, contexts[m][{{}, source_l}], table.unpack(rnn_state_decs[m])}
        end      

        local out_decoder = models[m][2]:forward(decoder_input)
        local out = models[m][3]:forward(out_decoder[#out_decoder]) -- K x vocab_size

      
        rnn_state_decs[m] = {} -- to be modified later
        if models_opt[m].input_feed == 1 then
	       table.insert(rnn_state_decs[m], out_decoder[#out_decoder])
        end      
      
        for j = 1, #out_decoder - 1 do
	       table.insert(rnn_state_decs[m], out_decoder[j])
        end

        -- get pure probabilities
        local renorm = (torch.exp(out)/torch.sum(torch.exp(out))):float()
        --local renorm = out:float()
	if m == 1 then
          out_float:resize(renorm:size()):copy(renorm)
        else
          out_float:add(renorm)
        end
      end
      out_float:div(models_num)
    
      -- convert into log again
      out_float = torch.log(out_float)
      for k = 1, K do
      ...
      -- All the scores available.
       ...
       for m = 1, models_num do
        ...
        for j = 1, #rnn_state_decs[m] do
	       rnn_state_decs[m][j]:copy(rnn_state_decs[m][j]:index(1, prev_ks[i]))
        end
       end
   ...
   if opt.score_gold == 1 then
      ...
      for m = 1, models_num do
        ...
        for i = 1, #init_fwd_decs[m] do
          table.insert(rnn_state_decs[m], init_fwd_decs[m][i][{{1}}]:zero())
        end
       
        if models_opt[m].init_dec == 1 then
          for L = 1, models_opt[m].num_layers do
	         rnn_state_decs[m][L*2-1+models_opt[m].input_feed]:copy(rnn_state_encs[m][L*2-1][{{1}}])
	         rnn_state_decs[m][L*2+models_opt[m].input_feed]:copy(rnn_state_encs[m][L*2][{{1}}])
	  end
       end
      end				   
      ...
      for t = 2, target_l do
        local decoder_input_words = gold[{{t-1}}]
	local g_tmp = 0
        for m = 1, models_num do
	  ...
	  if m %2 == 1 then
          ...
          if models_opt[m].attn == 1 then
	          decoder_inputs = {decoder_input_words, contexts[m][{{1}}], table.unpack(rnn_state_decs[m])}
	        else
	          decoder_inputs = {decoder_input_words, contexts[m][{{1}, source_l}], table.unpack(rnn_state_decs[m])}
	        end

	       local out_decoder = models[m][2]:forward(decoder_inputs)
	       local out = models[m][3]:forward(out_decoder[#out_decoder]) -- K x vocab_size
          ...
          table.insert(rnn_state_decs[m], out_decoder[j])
           ...
           g_tmp = g_tmp + torch.exp(out[1][gold[t]])/torch.sum(torch.exp(out[1]))
	   --g_tmp = g_tmp + out[1][gold[t]]
	end
        gold_score = gold_score + torch.log(g_tmp/models_num)
	--gold_score = gold_score + g_tmp
...
function main()
  ...
  if opt.gpuid2 == -1 then
      opt.gpuid2 =opt. gpuid
  end

  local model_paths = {}
  table.insert(model_paths, opt.model1)
  table.insert(model_paths, opt.model2)
  table.insert(model_paths, opt.model3)
  table.insert(model_paths, opt.model4)
  ...
  for m=1 , opt.models_num do
    assert(path.exists(model_paths[m], 'model does not exist'))
  end  
  ...
  for m=1 , opt.models_num do
    ...
    print('loading ' .. model_paths[m] .. '...')
    
    table.insert(checkpoints, torch.load(model_paths[m]))
    
    table.insert(models, checkpoints[m][1])
    table.insert(models_opt, checkpoints[m][2])

    for i = 1, #models[m] do
      models[m][i]:evaluate()
      if opt.gpuid >=0 then
	models[m][i]:double():cuda()
        models[m][i]:evaluate()
      end
    end

    -- for backward compatibility
    models_opt[m].brnn = models_opt[m].brnn or 0
    models_opt[m].input_feed = models_opt[m].input_feed or 1
    models_opt[m].attn = models_opt[m].attn or 1


    table.insert(context_protos, torch.zeros(1, MAX_SENT_L, models_opt[m].rnn_size))
    table.insert(h_init_decs, torch.zeros(opt.beam, models_opt[m].rnn_size))
    table.insert(h_init_encs, torch.zeros(1, models_opt[m].rnn_size))
    if opt.gpuid >= 0 then
      h_init_encs[m] = h_init_encs[m]:cuda()      
      h_init_decs[m] = h_init_decs[m]:cuda()
      context_protos[m] = context_protos[m]:cuda()
    end
    ...
    if models_opt[m].input_feed == 1 then
    	table.insert(init_fwd_decs[m], h_init_decs[m]:clone())
    end
    
    for L = 1, models_opt[m].num_layers do
      table.insert(init_fwd_encs[m], h_init_encs[m]:clone())
      table.insert(init_fwd_encs[m], h_init_encs[m]:clone())
      table.insert(init_fwd_decs[m], h_init_decs[m]:clone()) -- memory cell
      table.insert(init_fwd_decs[m], h_init_decs[m]:clone()) -- hidden state      
    end
  end
  ...
  if opt.replace_unk == 1 then
   ...
   for line in file:lines() do
      ...
      if models_opt[1].use_chars_enc == 0 then
	 source, source_str = sent2wordidx(line, word2idx_src, models_opt[1].start_symbol)
      end
      if opt.score_gold == 1 then
	 target, target_str = sent2wordidx(gold[sent_id], word2idx_targ, 1)
      end
      state = State.initial(START)
      pred, pred_score, attn, gold_score, all_sents, all_scores, all_attn = generate_beam(models, opt.models_num,
