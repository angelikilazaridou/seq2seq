...
function train(train_data, valid_data)
   ...
   local sampler_proto = torch.zeros(opt.max_batch_l, opt.z_size*2*opt.num_layers)
   hc_proto = torch.zeros(opt.max_batch_l, opt.num_layers *2 * opt.rnn_size)
   place_holder = torch.zeros(opt.max_batch_l, opt.rnn_size)
   ...
   function reset_state(state, batch_l, t)
	 ...
	 for i = 1, #state do
	    state[i]:zero()
	    table.insert(u, state[i][{{1, batch_l}}]:clone())
	 end
	 return u
      else
	 local u = {[t] = {}}
	 for i = 1, #state do
	    state[i]:zero()
	    table.insert(u[t], state[i][{{1, batch_l}}]:clone())
	 end
	 return u
      end      
   end
   ...
   function train_batch(data, epoch)
	 ...
	 local dz = sampler_proto[{{1,batch_l}, {1, 2*opt.num_layers*opt.z_size}}]:clone() 
	 ...
	 local hiddens = hc_proto[{{1, batch_l}, {1, 2 * opt.rnn_size}}]:clone()
	 local cells = hc_proto[{{1, batch_l}, {1, 2*  opt.rnn_size}}]:clone()
	 
	 local hidden_source = hc_proto[{{1, batch_l}, {1, opt.rnn_size}}]:clone()
	 local cell_source = hc_proto[{{1, batch_l}, {1,  opt.rnn_size}}]:clone()
         
	 local d_source = hc_proto[{{1, batch_l}, {1, 2 * opt.rnn_size}}]:clone()
	 local d_target = hc_proto[{{1, batch_l}, {1, 2 * opt.rnn_size}}]:clone()
	 ...
	 -- copy c and h from last timestep of each layer
         -- cells
         cells[{{1, batch_l}, {start_idx, end_idx}}]:copy(rnn_state_enc_source[source_l][1])-- source
         cells[{{1, batch_l}, {start_idx + opt.rnn_size, end_idx + opt.rnn_size}}]:copy(rnn_state_enc_target[target_l][1]) -- target
            
         -- hiddens
         hiddens[{{1, batch_l}, {start_idx, end_idx}}]:copy(rnn_state_enc_source[source_l][2])-- source
         hiddens[{{1, batch_l}, {start_idx + opt.rnn_size, end_idx + opt.rnn_size}}]:copy(rnn_state_enc_target[target_l][2])--target

         -- for source
         cell_source[{{1, batch_l}, {start_idx, end_idx}}]:copy(rnn_state_enc_source[source_l][1])-- source
         hidden_source[{{1, batch_l}, {start_idx, end_idx}}]:copy(rnn_state_enc_source[source_l][2])-- source
      

         -- use x and y to predict m and sigma from recognition model
	 local stats_phi = recognition_model:forward({cells, hiddens})
	 ...
	 local stats_omega = prior_model:forward({cell_source, hidden_source})

	 local mu_omega = stats_omega[1]
	 local logsigma_omega = stats_omega[2]
	

	 -- init decoders with hidden and cell from recognition model
	 local rnn_state_dec = reset_state(init_fwd_dec, batch_l, 0)
	 if opt.init_dec == 1 then
	    	rnn_state_dec[0][1]:copy(z[{{1,batch_l}, {start_idx, end_idx}}]) -- z for cell
		rnn_state_dec[0][2]:copy(z[{{1,batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}]) -- z for hidden
	 end	
	 ...
	 for t = 1, target_l do
	    ...
	    local decoder_input
	    decoder_input = {target[t], place_holder[{{1, batch_l}, {1, opt.rnn_size}}], table.unpack(rnn_state_dec[t-1])}
	 ...
	 dz:zero()
	 dz[{{1,batch_l},{start_idx, end_idx}}]:copy(drnn_state_dec[1]) -- gradients for z cell
	 dz[{{1,batch_l},{start_idx + opt.z_size, end_idx + opt.z_size}}]:copy(drnn_state_dec[2]) -- gradients for z history
	 ...
	 local d_hc_source_target = recognition_model:backward({cells, hiddens}, {dstats[1],  dstats[2]})
         --[[ 
	 print('d of recognition cells '.. d_hc_source_target[1]:norm())
	 print('d of recognition cells source '..d_hc_source_target[1][{{1, batch_l},{1,30}}]:norm())
	 print('d of recognition cells target '..d_hc_source_target[1][{{1, batch_l},{31,60}}]:norm()) 
	 print('d of recognition hiddens '..d_hc_source_target[2]:norm())
 	 print('d of recognition hiddens source '..d_hc_source_target[2][{{1, batch_l},{1,30}}]:norm())
	 print('d of recognition hiddens target '..d_hc_source_target[2][{{1, batch_l},{31,60}}]:norm())
         --]]
	 -- backward prop thourgh prior model
	 local d_hc_source = prior_model:backward({cell_source, hidden_source}, {dstats[3], dstats[4]})

         d_source:zero()
	 d_target:zero()

	 d_source[{{1, batch_l}, {start_idx, end_idx}}]:copy(d_hc_source_target[1][{{1, batch_l}, {start_idx, end_idx}}])-- cell
	 d_source[{{1, batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}]:copy(d_hc_source_target[2][{{1, batch_l}, {start_idx, end_idx}}])-- hidden

         d_target[{{1, batch_l}, {start_idx, end_idx}}]:copy(d_hc_source_target[1][{{1, batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}])-- cell
	 d_target[{{1, batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}]:copy(d_hc_source_target[2][{{1, batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}])--  hidden 
	 
	 --add to that gradients from prior model
         d_source[{{1, batch_l}, {start_idx, end_idx}}]:add(d_hc_source[1]) -- cell
	 d_source[{{1, batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}]:add(d_hc_source[2])-- hidden
	 ...
	 -- copy over hidden states
	 if opt.init_dec == 1 then
	      -- cell
	      drnn_state_enc_source[1]:copy(d_source[{{1, batch_l},{start_idx, end_idx}}])    -- source
	      drnn_state_enc_target[1]:copy(d_target[{{1, batch_l},{start_idx, end_idx}}])    -- target
	      -- hidden
	      drnn_state_enc_source[2]:copy(d_source[{{1, batch_l},{start_idx + opt.z_size, end_idx + opt.z_size}}])       -- source
	      drnn_state_enc_target[2]:copy(d_target[{{1, batch_l},{start_idx + opt.z_size, end_idx + opt.z_size}}])       -- target
...
function eval(data)
   ...
   for i = 1, data:size() do
      ...
      local hc_source = hc_proto[{{1, batch_l}, {1, 2 * opt.rnn_size}}]:clone()
     -- forward prop encoder source
      for t = 1, source_l do
	 ...
	 rnn_state_enc_source = out
      ...
      -- use x to predict m and s from prior model
      local stats_omega = prior_model:forward({rnn_state_enc_source[1],rnn_state_enc_source[2]})
      local mu_omega = stats_omega[1]
      local logsigma_omega = stats_omega[2]
      -- calculate z
      ...
      if opt.init_dec == 1 then
          local start_idx = 1
          local end_idx = opt.z_size
          rnn_state_dec[1]:copy(z[{{1,batch_l}, {start_idx, end_idx}}]) -- z for cell
          rnn_state_dec[2]:copy(z[{{1,batch_l}, {start_idx + opt.z_size, end_idx + opt.z_size}}]) -- z for hidden
      end
      
      local loss = 0
      for t = 1, target_l do
	 local decoder_input
	 decoder_input = {target[t], place_holder[{{1, batch_l}, {1, opt.rnn_size}}],  table.unpack(rnn_state_dec)}
...
function main() 
   ...
   if opt.train_from:len() == 0 then
      ...
      recognition_model = recognition_model_correct(opt.rnn_size*2, opt.z_size) 
      -- AL: prior model
      prior_model = prior_model_correct(opt.rnn_size, opt.z_size)
      -- AL: KDloss
